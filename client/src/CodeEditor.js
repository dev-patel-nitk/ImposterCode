      ...
      <div className="sidebar">
        ...
        <div className="sidebar-footer">
          <div className="footer-buttons" style={{ display: 'flex', gap: '10px' }}>
            <button className="copy-btn" onClick={() => navigator.clipboard.writeText(code)} style={{ flex: 1 }}>Copy Code</button>
            <button className="reset-btn" onClick={() => {
              setCode(BOILERPLATES[language] || "");
              socket.emit("code-change", { roomId, code: BOILERPLATES[language] || "" });
            }} style={{ flex: 1 }}>Reset</button>
          </div>
          <div className="exit-buttons">
            {isHost ? (
              <button className="end-btn" onClick={() => {
                 if (window.confirm("End meeting?")) { socket.emit("end-room", { roomId }); onLeave(); }
              }}>End Meeting</button>
            ) : (
      ...
      <div className="main-editor">
        ...
        <div className="editor-area">
          ...
          <div className="terminal-panel">
            ...
            <pre className="terminal-content">{output || "Click 'Run Code' to see output here..."}</pre>
          </div>
        </div>
      </div>
    </div>
