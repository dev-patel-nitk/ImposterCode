...
import useSabotage from "./useSabotage"; // Hook is already written — just importing
...
const SABOTAGE_COOLDOWN_SECS = 30;
...
const CodeEditor = ({ socket, roomId, username, isHost, onLeave }) => {
  ...
  // ✅ Sabotage cooldown state — purely for UI feedback.
  // The authoritative cooldown lives on the server. This timer
  // starts only when the server confirms the sabotage was accepted.
  ...
  // ✅ Sabotage hook — provides the three attack functions.
  // These operate on Monaco's view layer (markers, cursor, theme),
  // never on the Yjs document, so they are always ephemeral.
  ...
  // --- SABOTAGE COOLDOWN TICKER ---
  // Counts down sabotageCooldown by 1 each second.
  // Only runs when there is actually time left — no unnecessary intervals.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    socket.on("game-started", ({ duration, impostorId, meetingsLeft, questionIndex }) => {
      ...
      // Reset cooldown UI at game start so Impostor can sabotage immediately.
    ...
    // --- SABOTAGE INCOMING (Crewmate side) ---
    // These fire when the server broadcasts an attack to everyone except the Impostor.
    socket.on("sabotage-linter", () => triggerLinterLie());
    socket.on("sabotage-drift", () => triggerKeyboardDrift());
    socket.on("sabotage-blackout", () => triggerThemeBlackout());

    // ✅ SABOTAGE CONFIRMED (Impostor side)
    // The server sends this back to the Impostor after a successful,
    // validated sabotage. We start the UI cooldown timer here — not on
    // button click — so the client stays perfectly in sync with the server's
    // timestamp even if there's network latency.
    socket.on("sabotage-confirmed", ({ cooldownMs }) => {
      setSabotageCooldown(Math.ceil(cooldownMs / 1000));
    });

    // ✅ SABOTAGE REJECTED (Impostor side — tampered client only)
    // An honest client never sees this because the React UI prevents early
    // emission. If a hacked client bypasses the UI and emits early, the
    // server rejects it and we surface the remaining cooldown as an alert.
    // This also re-syncs the UI timer in case it somehow drifted.
    socket.on("sabotage-rejected", ({ secondsLeft }) => {
      console.warn(`[Sabotage] Rejected by server. ${secondsLeft}s remaining.`);
      setSabotageCooldown(secondsLeft); // Re-sync UI to server's actual remaining time.
      alert(`⚠️ Sabotage on cooldown! ${secondsLeft}s remaining.`);
    });
    ...
    return () => {
      ...
      socket.off("sabotage-linter");
      socket.off("sabotage-drift");
      socket.off("sabotage-blackout");
      socket.off("sabotage-confirmed");
      socket.off("sabotage-rejected");
      if (typingTimeoutRef.current) clearTimeout(typingTimeoutRef.current);
    };
  }, [socket, roomId, currentQuestion, triggerLinterLie, triggerKeyboardDrift, triggerThemeBlackout]);
  ...
  const handleEditorDidMount = (editor, monaco) => {
    ...
    const provider = new SocketIOProvider(SOCKET_URL, roomId, doc, { autoConnect: true });
  ...
  const handleStartGame = () => {
    socket.emit("start-game", { roomId, duration: gameDuration, meetingDuration });
  };
  ...
  // ✅ Sabotage trigger — emits to server and lets server response drive the UI.
  // The cooldown state is NOT set here; it's set in the "sabotage-confirmed" handler.
  // This way the UI timer is always anchored to the server's actual timestamp.
  const handleSabotage = (type) => {
    if (sabotageCooldown > 0) return; // UX guard — server will also reject, but no need to spam it.
    socket.emit("trigger-sabotage", { roomId, type });
  };
  ...
  // ✅ Sabotage Menu — only rendered for the Impostor during an active game.
  // Buttons are disabled while sabotageCooldown > 0.
  // The cooldown display updates every second via the useEffect ticker above.
  const renderSabotageMenu = () => {
    if (role !== "Impostor" || gameStatus !== "running" || isEjected) return null;
    ...
    return (
      <div style={{
        ...
        <h4 style={{ color: 'var(--neon-red)', margin: '0 0 12px 0', textAlign: 'center', fontSize: '0.85rem', letterSpacing: '2px' }}>
          ☠️ SABOTAGE CONSOLE
        </h4>

        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          <button
            onClick={() => handleSabotage("linter")}
            disabled={onCooldown}
            style={sabotageButtonStyle(onCooldown)}
          >
            🔴 LINTER LIE
          </button>
          <button
            onClick={() => handleSabotage("drift")}
            disabled={onCooldown}
            style={sabotageButtonStyle(onCooldown)}
          >
            🌀 KEYBOARD DRIFT
          ...
          <button
            onClick={() => handleSabotage("blackout")}
            disabled={onCooldown}
            style={sabotageButtonStyle(onCooldown)}
          >
            🌑 THEME BLACKOUT
        ...
        {onCooldown && (
          <p style={{ textAlign: 'center', margin: '10px 0 0 0', color: '#ff6666', fontSize: '0.8rem', fontFamily: 'JetBrains Mono' }}>
            ⏳ COOLDOWN: {sabotageCooldown}s
          </p>
        )}
      </div>
    );
  ...
  const sabotageButtonStyle = (disabled) => ({
    padding: '9px',
    background: disabled ? 'rgba(255,17,0,0.05)' : 'rgba(255,17,0,0.15)',
    border: `1px solid ${disabled ? '#550000' : 'var(--neon-red)'}`,
    color: disabled ? '#550000' : 'var(--neon-red)',
    borderRadius: '4px',
    ...
    <div className={`editor-wrapper ${theme}`} style={{ display: "flex", width: "100vw", height: "100vh", overflow: "hidden" }}>
        ...
        <div className="sidebar-content" style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
          {activeTab === 'problem' ? (
            <div style={{ flex: 1, overflowY: 'auto', padding: '10px' }}>
              ...
              {/* ✅ Sabotage Menu renders here, below the problem panel */}
              {renderSabotageMenu()}
              ...
              {gameStatus === "running" && !isEjected && (
                <div style={{ marginTop: '20px', textAlign: 'center' }}>
                  <button
                    ...
                    style={{ width: '100%', padding: '12px', background: 'var(--neon-red)', color: 'white', border: 'none', borderRadius: '4px', fontWeight: 'bold', cursor: meetingsLeft > 0 ? 'pointer' : 'not-allowed', opacity: meetingsLeft > 0 ? 1 : 0.5 }}
                  >
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
              <div className="users-list-compact" style={{ maxHeight: '120px', overflowY: 'auto', borderBottom: '1px solid #333', padding: '10px' }}>
                ...
                <ul style={{ margin: 0, padding: 0, listStyle: 'none' }}>
                  {users.map((u) => (
                    <li key={u.id} style={{ color: getUserColor(u.username), fontSize: '0.9rem', padding: '2px 0', opacity: kickedIds.includes(u.id) ? 0.5 : 1, textDecoration: kickedIds.includes(u.id) ? 'line-through' : 'none' }}>
                      ● {u.username} {kickedIds.includes(u.id) ? "(EJECTED)" : ""}
                    </li>
                  ))}
                </ul>
              </div>
      ...
      <div className="main-editor" style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        <div className="toolbar" style={{ flexShrink: 0 }}>
          ...
          <div className="toolbar-group">
            <button className="theme-toggle" onClick={() => setTheme((prev) => (prev === "vs-dark" ? "light" : "vs-dark"))}>{theme === "vs-dark" ? "☀️ Light" : "🌙 Dark"}</button>
          </div>
        ...
        <div className="editor-area" style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', position: 'relative' }}>
          {isEjected && (
            <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.85)', zIndex: 100, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', color: 'var(--neon-red)', fontFamily: 'Orbitron' }}>
              <h1 style={{ fontSize: '3rem', margin: 0, textShadow: '0 0 20px red' }}>👻 YOU ARE DEAD</h1>
