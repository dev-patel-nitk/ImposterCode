...
const CodeEditor = ({ socket, roomId, username, isHost, onLeave }) => {
  ...
  useEffect(() => {
    ...
    // 🟢 SECURE QUESTION LISTENER: Receives the full object from the server
    socket.on("question-update", (questionData) => {
    ...
    socket.on("user-typing", ({ username, color }) => {
      if (typingTimeoutsRef.current.has(username)) clearTimeout(typingTimeoutsRef.current.get(username));
    ...
    // 🟢 GAME START LISTENER: Receives the full question object, not an index
    socket.on("game-started", ({ duration, impostorId, meetingsLeft, questionData }) => { 
    ...
    socket.on("game-over", ({ result, impostorUsername }) => {
      if (result === "CREWMATE_WIN" || result === "crewmates_escaped") playSound("VICTORY");
  ...
  const testAcidTransaction = async () => {
    const currentUserStr = localStorage.getItem("imposter_user");
    if (!currentUserStr) return alert("You must be logged in to test this!");
    const currentUser = JSON.parse(currentUserStr);

    alert("Firing 5 simultaneous requests to add 10 XP each...");
    const reqs = [];
    for(let i = 0; i < 5; i++) {
      reqs.push(fetch("/api/users/add-xp", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ username: currentUser.username, xpToAdd: 10 })
      }));
    }
    const results = await Promise.all(reqs);
    console.log("Transaction Results:", results);
    alert("All 5 requests resolved! Check your profile or the DB, XP should have increased exactly by 50 thanks to PostgreSQL Row Locking!");
  };
  
  // 🟢 SECURE START GAME: Fetch random problem from server, then broadcast it
  const handleStartGame = async () => { 
    try {
      const response = await fetch('/api/questions/generate');
      if (!response.ok) throw new Error("Failed to fetch mission from mainframe.");
      const secureProblem = await response.json();

      socket.emit("start-game", { 
        roomId, 
        duration: gameDuration, 
        meetingDuration: meetingDuration,
        questionData: secureProblem // Send the full object to the socket server
      }); 
    } catch (err) {
      alert(err.message);
      console.error(err);
  ...
  const submitCode = () => {
    ...
    const currentCode = editorRef.current.getValue();
    socket.emit("submit-code", { roomId, language, code: currentCode });
  };
            ...
            >
              ...
              {demoSolutionText && (
                <button
                  type="button"
                  onClick={(e) => { 
                    navigator.clipboard.writeText(demoSolutionText);
          ...
          {typingUsers.size > 0 && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '5px', marginLeft: '8px', overflow: 'hidden', flexShrink: 0 }}>
              {[...typingUsers.entries()].map(([name, color]) => (
                <span key={name} style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', background: `${color}20`, border: `1px solid ${color}66`, borderRadius: '999px', padding: '1px 8px 1px 5px', fontSize: '0.7rem', color: color, fontFamily: 'Orbitron', whiteSpace: 'nowrap', animation: 'fadeIn 0.1s ease' }}>
